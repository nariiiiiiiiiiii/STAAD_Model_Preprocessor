...
User-selected Project JSON First Save/Save As destinations are **HIGH-RISK** because a wrong target or overwrite can lose user model data. The owner historically approved STRICT / Full TDD on **2026-09-12**. On 2026-09-13 the owner confirmed Save still blocks an external folder and requested a refreshed plan before approving implementation. Source inspection traces the popup to the First Save-only `_assert_project_json_path()` call after the chooser and before atomic serialization. No save-path implementation has started; wait for approval of the refreshed plan at `docs/superpowers/plans/2026-09-12-user-selected-save-destinations.md`.
