...
Next usability follow-up: **SOURCE VERIFIED; TASKBAR LOGO OWNER-CONFIRMED; VIEWPORT MANUAL ACCEPTANCE PASS; SAVE BLOCK CONFIRMED / PLAN APPROVAL PENDING; PACKAGE ACCEPTANCE PENDING**. CP1–CP5 implement source version `0.2.0`, owner-confirmed SketchUp JSON naming, the selected Qt/executable icon, unrestricted Project JSON Open with Save-back, and STRICT multi-issue Quick Fix/intersection routing. Evidence: **345 unit+integration PASS**, **1 RBZ archive test skipped**, **3 portable-executable gates deselected**, **26/26 affected UI PASS**, Ruff PASS, strict mypy **0 issues** for four source files, and diff check PASS. On 2026-09-12 the owner reported that the app-window and source-run taskbar icons now show the selected logo, and Quick Fix/Undo/Redo work. The source taskbar change has **10/10 focused checks PASS**. The owner reports Crop to Select / Zoom in Select work on 2026-09-13. Source inspection maps the remaining external Project JSON Save Blocked dialog to `_assert_project_json_path()` in the First Save branch. The owner requests a refreshed plan and will approve before implementation; the historical STRICT approval on 2026-09-12 is recorded. Import dialogs and Export STD already accept chosen external paths; the STD validation report stays next to the selected `.STD`. The compiled executable is still `0.1.0.0` from 2026-09-06; no `0.2.0` executable/RBZ/ZIP has been built or accepted.

Additional 2026-09-12 request, owner recheck 2026-09-13: **Crop to Select / Zoom in Select is SOURCE IMPLEMENTED and manually accepted by the owner.** The **13 non-renderer event/action/geometry tests PASS**, Ruff passes, and strict mypy reports **0 issues** in the two changed source files. Automated real-VTK smoke remains unverified because offscreen startup reported `vtkWin32OpenGLRenderWindow: failed to get valid pixel format`, followed by a Python Application Error. Save remains blocked for external Project JSON paths; source traces it to the First Save path guard. The refreshed STRICT plan is awaiting owner approval; no Save-path code or release build has been changed.
...
- [`superpowers/plans/2026-09-12-user-selected-save-destinations.md`](superpowers/plans/2026-09-12-user-selected-save-destinations.md) — Save Blocked root cause and STRICT First Save/Save As plan; awaiting owner's refreshed-plan approval.
...
- [`superpowers/specs/2026-09-12-user-selected-save-destinations.md`](superpowers/specs/2026-09-12-user-selected-save-destinations.md) — confirmed First Save guard, unrestricted explicit save scope, atomic-write safety, and current approval gate.
