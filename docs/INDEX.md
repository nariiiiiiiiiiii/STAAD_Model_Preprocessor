...
Next usability follow-up: **SOURCE VERIFIED; PARTIAL DEVELOPMENT-APP SMOKE; TASKBAR LOGO OWNER-CONFIRMED; CROP SOURCE FIX VERIFIED; SAVE WORKFLOW CLARIFICATION PENDING; PACKAGE ACCEPTANCE PENDING**. CP1–CP5 implement source version `0.2.0`, owner-confirmed SketchUp JSON naming, the selected Qt/executable icon, unrestricted Project JSON Open with Save-back, and STRICT multi-issue Quick Fix/intersection routing. Evidence: **345 unit+integration PASS**, **1 RBZ archive test skipped**, **3 portable-executable gates deselected**, **26/26 affected UI PASS**, Ruff PASS, strict mypy **0 issues** for four source files, and diff check PASS. On 2026-09-12 the owner reported that the app-window and source-run taskbar icons now show the selected logo, and Quick Fix/Undo/Redo work. The source taskbar change has **10/10 focused checks PASS**. The new single-Node Crop-to-Selection regression now passes in the isolated real-PyVista smoke; the combined focused viewport and Project-save suite is **15/15 PASS**, Ruff PASS, strict mypy **0 issues** in the affected viewport source, and diff check passes. The Save Blocked message maps to the intentional first-save Projects guard unless the owner had opened an external Project JSON first; no path behavior was changed while this remains unclarified. The compiled executable is still `0.1.0.0` from 2026-09-06; no `0.2.0` executable/RBZ/ZIP has been built or accepted.
