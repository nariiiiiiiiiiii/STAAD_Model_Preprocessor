...
Next usability follow-up: **SOURCE VERIFIED; PARTIAL DEVELOPMENT-APP SMOKE; TASKBAR SOURCE FIX ADDED; OWNER/PACKAGE ACCEPTANCE PENDING**. CP1–CP5 implement source version `0.2.0`, owner-confirmed SketchUp JSON naming, the selected Qt/executable icon, unrestricted Project JSON Open with Save-back, and STRICT multi-issue Quick Fix/intersection routing. Evidence: **345 unit+integration PASS**, **1 RBZ archive test skipped**, **3 portable-executable gates deselected**, **26/26 affected UI PASS**, Ruff PASS, strict mypy **0 issues** for four source files, and diff check PASS. On 2026-09-12 the owner reported that the app-window icon changed and Quick Fix/Undo/Redo work, but the taskbar still displayed the old/Python-looking icon. Source now sets a stable Windows AppUserModelID before QApplication creation and explicitly assigns the app icon to the main window; the focused follow-up is **10/10 PASS**, Ruff PASS, and strict mypy **0 issues** across two source files. Live taskbar retest remains pending because no Windows app window was exposed to this session. The existing executable is still version `0.1.0.0` from 2026-09-06; no `0.2.0` executable/RBZ/ZIP has been built or accepted. A legacy test briefly generated a transient RBZ, which was removed; the fixture tests no longer build under `build/`.
