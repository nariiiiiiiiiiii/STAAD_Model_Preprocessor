...
Next usability follow-up: **SOURCE VERIFIED; PARTIAL DEVELOPMENT-APP SMOKE; TASKBAR LOGO OWNER-CONFIRMED; CROP SOURCE FIX VERIFIED; SAVE DESTINATIONS STRICT-APPROVED/QUEUED; PACKAGE ACCEPTANCE PENDING**. CP1–CP5 implement source version `0.2.0`, owner-confirmed SketchUp JSON naming, the selected Qt/executable icon, unrestricted Project JSON Open with Save-back, and STRICT multi-issue Quick Fix/intersection routing. Evidence: **345 unit+integration PASS**, **1 RBZ archive test skipped**, **3 portable-executable gates deselected**, **26/26 affected UI PASS**, Ruff PASS, strict mypy **0 issues** for four source files, and diff check PASS. On 2026-09-12 the owner reported that the app-window and source-run taskbar icons now show the selected logo, and Quick Fix/Undo/Redo work. The source taskbar change has **10/10 focused checks PASS**. The single-Node Crop-to-Selection source fix and current marquee source are implemented. The owner explicitly requested any-folder Project JSON First Save/Save As and approved STRICT / Full TDD; this high-risk work is queued until the viewport checkpoint is reviewed. Import dialogs and Export STD already accept chosen external paths; the STD validation report stays next to the selected `.STD`. The compiled executable is still `0.1.0.0` from 2026-09-06; no `0.2.0` executable/RBZ/ZIP has been built or accepted.

Additional 2026-09-12 request: **Crop to Select marquee + Zoom in Select rename is SOURCE IMPLEMENTED; owner/real-VTK acceptance pending.** The **13 non-renderer event/action/geometry tests PASS**, Ruff passes, and strict mypy reports **0 issues** in the two changed source files. The real-VTK smoke could not be confirmed in this session: offscreen startup reported `vtkWin32OpenGLRenderWindow: failed to get valid pixel format`, followed by a Python Application Error. The owner approved STRICT / Full TDD for user-selected Save destinations; implementation is queued until the viewport checkpoint is reviewed. No Save-path code or release build has been changed.
...
- [`superpowers/plans/2026-09-12-marquee-selection.md`](superpowers/plans/2026-09-12-marquee-selection.md) — Crop to Select marquee / Zoom in Select; non-renderer checks pass, VTK smoke pending.
- [`superpowers/plans/2026-09-12-user-selected-save-destinations.md`](superpowers/plans/2026-09-12-user-selected-save-destinations.md) — STRICT-approved First Save/Save As path changes; queued behind viewport acceptance.
...
- [`superpowers/specs/2026-09-12-user-selected-save-destinations.md`](superpowers/specs/2026-09-12-user-selected-save-destinations.md) — unrestricted explicit save destinations, preserved atomic writes, and approval record.
